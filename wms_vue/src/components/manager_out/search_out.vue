...
<!-- 查询组件 -->
      ...
      <el-input v-model="deposity" placeholder="请输入目的仓库" style="width:20%; margin-left: 10px"></el-input>
  ...
  <div style="margin-top: 20px;margin-bottom: 10px">
    ...
    <el-upload action="http://localhost:9000/recordOut/import" :headers="{token:user.token}" :on-success="handleImport" style="display:inline-block ;margin-left: 10px" :show-file-list="false">
  ...
  created(){
    ...
    methods: {
      ...
      if(!this.record_ids.length){//没有选择行的时候，全部导出，或者根据搜索条件查询到的数据全部导出
        window.open('http://localhost:9000/recordOut/export?token='+ this.user.token + "&pname="+this.pname + "&deposity="+this.deposity )
      }else{
        let record_idsStr = this.record_ids.join(',')
        window.open('http://localhost:9000/recordOut/export?token='+ this.user.token + "&record_ids="+record_idsStr)//传了选中的行
      }
      },
      ...
      if(pageNum)
      {
        ...
        request.get('/recordOut/selectByPage', {
          params: {
