...
public class RecordOutController {
    ...
    public ResponseResult getDate(){
        ...
        List <recordOut> all=recordOutService.findAllRecordOut();
...
//        return ResponseResult.success(recordInService.findAllRecordIn());
     ...
     */
    @PutMapping("/update")
    public ResponseResult updateRecord(@RequestBody recordOut record)
    {
        return ResponseResult.success(recordOutService.updateById(record));
    }
    /**
     * 新增
     */
    @PostMapping("/add")
    public ResponseResult addRecord(@RequestBody recordOut record)
    {
        return ResponseResult.success(recordOutService.save(record));
    }
    ...
    public ResponseResult selectByPage(@RequestParam Integer pageNum,
        ...
        QueryWrapper<recordOut> queryWrapper = new QueryWrapper<recordOut>().orderByDesc("record_id");
        ...
        IPage<recordOut> page =new Page<>(pageNum,pageSize);
        return ResponseResult.success(recordOutService.page(page,queryWrapper));
    }
    ...
    public void exportData(@RequestParam(required = false) String pname,
        ...
        QueryWrapper<recordOut> queryWrapper = new QueryWrapper<recordOut>();
        ...
        //全部导出
        list = recordOutService.list(queryWrapper);//查询出当前deposityRecord表的所有数据
        writer.write(list,true);

        //设置浏览器响应格式
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8");
        response.setHeader("Content-Disposition","attachment;filename="+ URLEncoder.encode("出库信息表","UTF-8") + ".xlsx");
    ...
    public ResponseResult importData(MultipartFile file) throws IOException {
        ...
        List<recordOut> recordList = reader.readAll(recordOut.class);

        boolean saveBatch;
        //写入数据到数据库
        try {
            saveBatch = recordOutService.saveBatch(recordList);
        }catch (Exception e){
            e.printStackTrace();
